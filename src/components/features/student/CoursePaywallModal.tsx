...
import { X, Lock, CheckCircle, Sparkles, Layers } from 'lucide-react';
...
interface CoursePaywallModalProps {
  courseName: string;
  courseId: string;
  classId: string; // Needed for bundle
  onClose: () => void;
}
  ...
  const handlePayment = async (type: 'single' | 'bundle') => {
    ...
    try {
      ...
      const res = await fetch('/api/payment/initialize', {
        ...
        body: JSON.stringify({
          type: 'class_unlock',
          plan: type === 'single' ? courseId : classId, // ID of what we are buying
          accessType: type // 'single' or 'bundle' helper for backend
        })
      });
      ...
      if (data.bypass) {
        toast.success("Test Coupon: Course Unlocked!");
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/80 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className="bg-white rounded-3xl p-0 w-full max-w-lg shadow-2xl relative overflow-hidden">
        ...
        <div className="bg-slate-900 p-8 text-center relative overflow-hidden">
          ...
          <p className="text-slate-400 mt-2 text-sm">You've completed the free trial week. Unlock the full course to continue.</p>
        </div>
        ...
        <div className="p-8 space-y-6">
          ...
          <div className="p-5 border-2 border-slate-100 rounded-2xl hover:border-indigo-100 transition-all relative group">
            ...
            >
              {loading === 'single' ? 'Processing...' : 'Unlock This Course'}
            </Button>
          </div>
            ...
            <div className="bg-white rounded-xl p-5">
              <div className="flex justify-between items-center mb-3">
                ...
                <div className="text-right">
                  <span className="text-xs text-slate-400 line-through mr-1">₵75</span>
              ...
              <ul className="space-y-2 mb-4">
                ...
                <li className="flex gap-2 text-xs text-slate-600 font-medium"><CheckCircle className="w-4 h-4 text-green-500"/> Download all PDF Handouts</li>
              </ul>
              <Button 
              ...
              >
                {loading === 'bundle' ? 'Processing...' : 'Get The Bundle'}
              </Button>
            </div>
          </div>
