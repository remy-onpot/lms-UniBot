    ...
    <div className="space-y-8">
      ...
      <section>
        <h2 className="text-lg font-bold text-slate-900 mb-4 flex items-center gap-2">
          <span className="text-red-700 bg-red-50 p-1.5 rounded-lg text-lg" aria-hidden="true">📕</span> Main Handout
        </h2>
        ...
        {mainHandout ? (
          ...
          canEdit ? (
            !isCourseRep ? (
              <label className="block w-full border-2 border-dashed border-slate-300 rounded-2xl p-8 text-center cursor-pointer hover:border-blue-500 hover:bg-blue-50 transition focus-within:ring-2 focus-within:ring-blue-500">
      ...
      <section>
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-lg font-bold text-slate-900 flex items-center gap-2">
            <span className="text-blue-700 bg-blue-50 p-1.5 rounded-lg text-lg" aria-hidden="true">📚</span> Resources
          </h2>
          {canEdit && !isCourseRep && (
            <label className="cursor-pointer text-xs font-bold text-blue-700 bg-blue-50 px-3 py-1.5 rounded-lg hover:bg-blue-100 transition focus-within:ring-2 focus-within:ring-blue-500">
              + Upload
          ...
          {supplementaryMaterials.map(m => (
            <div key={m.id} className="flex justify-between items-center p-4 bg-white rounded-xl border border-slate-200 shadow-sm hover:border-blue-300 transition">
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 bg-slate-100 rounded-lg flex items-center justify-center text-sm" aria-hidden="true">📑</div>
                <span className="text-sm font-medium text-slate-700 truncate max-w-[200px]">{m.title}</span>
              </div>
              <a href={m.file_url} target="_blank" rel="noreferrer" className="text-xs font-bold text-blue-700 hover:bg-blue-50 px-3 py-1.5 rounded-lg transition">Download</a>
            </div>
          ))}
