...
// ✅ FIX: Import Achievement from the service file
...
export function AchievementsTab({ achievements, onRefresh }: AchievementsTabProps) {
  ...
  const handleSync = async () => {
    ...
    try {
      ...
      const newUnlocks = await gameService.checkAndAwardAchievements(user.id, { type: 'streak_update' });

      if (newUnlocks && newUnlocks.length > 0) {
        toast.success(`Synced! Unlocked ${newUnlocks.length} badges.`);
      } else {
            ...
            >
              <div className="flex justify-between items-start mb-3">
                 <div className={`w-12 h-12 rounded-xl flex items-center justify-center transition-transform group-hover:scale-110 ${
                    isUnlocked ? 'bg-gradient-to-br from-yellow-50 to-orange-50 text-orange-500' : 'bg-slate-200 text-slate-400'
                 }`}>
                    <Icon className="w-6 h-6" />
                 </div>
                 {isUnlocked ? (
                    <span className="text-[10px] font-bold bg-green-100 text-green-700 px-2 py-1 rounded-full">
                       {ach.earned_at ? new Date(ach.earned_at).toLocaleDateString() : 'Unlocked'}
                    </span>
                 ) : (
              ...
              {/* ✅ FIX: These properties now exist on the type */}
              ...
              {isUnlocked && (
                 <div className="mt-3 inline-flex items-center gap-1 text-[10px] font-bold text-yellow-600 bg-yellow-50 px-2 py-0.5 rounded">
                    <Zap className="w-3 h-3" /> +{ach.points} XP
                 </div>
              )}
            </div>
          );
