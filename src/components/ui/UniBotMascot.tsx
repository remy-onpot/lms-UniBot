...
interface UniBotMascotProps {
  size?: number | string; // 🔄 Now accepts strings like "100%"
...
export function UniBotMascot({ 
  size, // 🔄 Removed default 120 here to detect if user passed a size
  emotion = 'idle', 
  action = 'none', 
  className,
  autoBlink = true 
  ...
  return (
    <div 
      className={cn("relative select-none perspective-500", className)}
      style={{ 
    ...
    >
      ...
      >
        ...
        <g className="origin-center">
          ...
          <g className={cn("transition-all duration-200 ease-spring origin-[100px_90px]", (internalAction === 'blink' || emotion === 'sleeping' || emotion === 'shy') ? "scale-y-[0.1]" : "scale-y-100")} filter={`url(#glow-${uniqueId})`}>
          ...
          <g fill="none" stroke={emotion === 'sad' ? '#60A5FA' : '#22D3EE'} strokeWidth="4" strokeLinecap="round" strokeLinejoin="round" filter={`url(#glow-${uniqueId})`} className="transition-opacity duration-300">
          ...
          <g className={cn("transition-all duration-500 origin-[165px_110px]", internalAction === 'wave' ? "animate-hand-wave" : "", emotion === 'shy' ? "translate-x-[-30px] translate-y-[-50px]" : "")}>
          ...
          <g transform="translate(130, 35)" className={cn("transition-transform duration-500 origin-bottom", emotion === 'happy' || internalAction === 'dance' ? "rotate-[-20deg] translate-y-[-5px]" : "rotate-0")}>
